				...
				for (int stepCount = _SamplingQuality; stepCount >= 0; --stepCount)
				{
					...
					if (distToCamera <= currDistToCamera)//if we aren't occluded by depth buffer objects
					{
						...
						if (_DoOcclusion != 0) { //if we are considering the occlusion plane
							...
							if (val <= 0) {
								//stop sampling once we cross into the occlusion space
								sample = false; 
								if (sampledAtLastPoint) break;
							}
						}

						fixed4 mask = sample ? tex3D(_MainTex, pos + 0.5f) : fixed4(0, 0, 0, 0);
						...
						if (sample && _DoCutting != 0) { //if we are considering the cutting planes
							...
							for (index; index < _NumCuttingPlanes; index++) {
								...
								float brightness = mask.a * ((mask.r + mask.g + mask.b) / 3);
								float dist = 10 * abs(val) / length(_CuttingPlaneNormals[index].xyz);
								...
								if (val <= 0)
								{
									...
									if (brightness < dist || _CuttingPlaneNormals[index].x > 0)
									{
										sample = false;
										break;
									}
